# Script: Rename-FPartitionBootEntry.ps1
    ...
    try {
        Invoke-Command -ComputerName $Computer -ScriptBlock {
            ...
            for ($i = 0; $i -lt $bcd.Count; $i++) {
                ...
                if ($bcd[$i] -match "device\s+partition=F:") {
                    $targetIds += $id
                }
            }

            if ($targetIds.Count -eq 0) {
                Write-Host "[$env:COMPUTERNAME] No partition=F: boot entries found." -ForegroundColor Yellow
            } else {
