...
function Get-MeshDevices($groupName) {
    $raw = & node $meshCtrlPath ListDevices --url $serverUrl --loginuser $username --loginpass $password
    $devices = @()
    $inGroup = $false
    $skip = 0
...
$goBtn.Add_Click({
    ...
    foreach ($pc in $hostList) {
        ...
        Invoke-AMTCommand -ComputerName $pc -Credential $amtCred -Operation $amtActions[$selectedOp].Native
    }
