...
$GUI.Text ="Enabler GUI by BrunoTheStudent"
$GUI.Width = 256
$GUI.Height = 256
...
$Copyright.Text = "Copyright © 2022 BrunoTheStudent"
...
$Operations = @('PowerOn','Install','Uninstall','Features','Boot','VMImport','AllPrograms','VMExists','SoftwareExists','WinActivation','WinStatus','ProjectRearm','GPUpdate','AddLanguage','RemoveProfile','DiskCleanup','Restart','Shutdown')
...
$OperationBox.Add_SelectedIndexChanged({
    ...
    $AdditionalList = @('Install','Uninstall','Features','Boot','VMImport','VMExists','SoftwareExists','AddLanguage','RemoveProfile')
...
$Execute.Add_Click(
{
    ...
    if(Test-Path "$Path\Resources\Hosts\$Choice.txt") {
        $Hosts = Get-Content "$Path\Resources\Hosts\$Choice.txt"
        $Operations = @('PowerOn','Install','Uninstall','Features','Boot','VMImport','NetAdapter','AllPrograms','VMExists','SoftwareExists','WinActivation','WinStatus','ProjectRearm','GPUpdate','AddLanguage','RemoveProfile','DiskCleanup','Restart','Shutdown','Exit')
                        ...
                        if(Test-Path "$Path\Resources\VM\$Choice") {
                            foreach ($PC in $Hosts) {
                                ...
                                Invoke-Command -ComputerName $PC -ScriptBlock {
                                    ...
                                    if($using:Choice -eq "MS Project VM") {
                                        Import-VM -Path 'D:\MS Project VM\Microsoft Project\Virtual Machines\60607131-69CD-45F2-8654-3B51CDB0FC6B.vmcx' -Copy -GenerateNewId -VirtualMachinePath "D:\MS Project VM\" -VhdDestinationPath "D:\MS Project VM\Virtual Hard Disks\" -SnapshotFilePath "D:\MS Project VM\" -SmartPagingFilePath "D:\MS Project VM\"    
                    ...
                    VMExists {
                        ...
                        foreach ($PC in $Hosts) {
                            ...
                            <#$Name=Invoke-Command -ComputerName $PC -ScriptBlock {
                                $Result=Get-VM  #| Where-Object -Property Name -Like "$using:Choice*"
                                } -Credential $using:Cred
                            return $Result
                            } -Credential $Cred#>
                            $Name=Get-VM -ComputerName $PC | Where-Object {$_.Name -like 'ASBP*'}
                            Write-Host $Name
                        }
                    } 
                    SoftwareExists {
